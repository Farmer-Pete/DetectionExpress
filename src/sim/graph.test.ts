...
/** The canonical four-node chain: ingest -> normalize -> detect -> sink. */
...
describe("validateLinearChain", () => {
  it("accepts the four-node chain and returns its ordered ids", () => {
    ...
    expect(result.nodeIds).toEqual(["ingest", "normalize", "detect", "sink"]);
  ...
  it("rejects an unknown node kind", () => {
    const nodes = chain.nodes.map((n) => (n.id === "detect" ? { id: "detect", kind: "bogus" } : n));
  ...
  it("rejects a wrong node count (a duplicate kind)", () => {
    const nodes = [...chain.nodes, { id: "detect2", kind: "detect" }];
    expect(() => validateLinearChain(nodes, chain.edges)).toThrow(/detect/i);
  });

  it("rejects a branch (a fourth edge off the chain)", () => {
    const edges = [...chain.edges, { id: "e4", source: "ingest", target: "detect" }];
    expect(() => validateLinearChain(chain.nodes, edges)).toThrow(/three edges/i);
  });

  it("rejects a wrong order (ingest wired straight to detect)", () => {
    const edges: GraphEdge[] = [
      { id: "e1", source: "ingest", target: "detect" },
      { id: "e2", source: "normalize", target: "detect" },
      { id: "e3", source: "detect", target: "sink" },
    ];
  ...
  it("rejects a cycle (an edge back to an earlier node)", () => {
    const edges: GraphEdge[] = [
      { id: "e1", source: "ingest", target: "normalize" },
      { id: "e2", source: "normalize", target: "detect" },
      { id: "e3", source: "detect", target: "normalize" },
    ];
  ...
  it("rejects duplicate edge ids", () => {
    const edges: GraphEdge[] = [
      ...
      { id: "same", source: "normalize", target: "detect" },
      { id: "e3", source: "detect", target: "sink" },
    ];
